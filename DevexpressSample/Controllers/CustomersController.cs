    ...
    public class CustomersController : Controller
    {
        ...
        private void PopulateModel(Customer model, IDictionary values) {
            ...
            if(values.Contains(CUSTOMER_ID)) {
                //model.CustomerId = Convert.ToInt64(values[CUSTOMER_ID]);
                long customerId = Convert.ToInt64(values[CUSTOMER_ID]);
                model.CustomerId = customerId + 100;
            }
            ...
            if(values.Contains(SUPPORT_REP_ID)) {
                model.SupportRepId = values[SUPPORT_REP_ID] != null ? Convert.ToInt64(values[SUPPORT_REP_ID]) : (long?)null;
            }
        }
